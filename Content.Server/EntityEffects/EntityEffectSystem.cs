...
public sealed partial class EntityEffectSystem : EntitySystem // SS220-add-partial-to-god-save-it
{
    ...
    public override void Initialize()
    {
        ...
        InitializeSS220(); // SS220-save-from-disaster
    ...
    private void OnExecuteFlammableReaction(ref ExecuteEntityEffectEvent<FlammableReaction> args)
    {
        ...
        if (HasComp<VehicleComponent>(args.Args.TargetEntity))
        {
            _explosion.QueueExplosion(args.Args.TargetEntity, "Default", 3, 25f, 10, canCreateVacuum: false); // Not deadly to rider but still painful
            if (TryComp<DamageableComponent>(args.Args.TargetEntity, out var damageablecomp))
            {
                var damageToVehicle = 600;
                _damageable.SetAllDamage(args.Args.TargetEntity, damageablecomp, damageToVehicle); // Hardcoded number but its enough to destroy any vehicle rn
            }
            return;
        }
